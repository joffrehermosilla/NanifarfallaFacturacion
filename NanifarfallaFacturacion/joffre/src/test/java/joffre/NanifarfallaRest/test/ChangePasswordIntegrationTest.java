...
import joffre.NanifarfallaRest.NanifarfallaRestApplication;
...
@SpringBootTest(classes = { NanifarfallaRestApplication.class, TestDbConfig.class, TestIntegrationConfig.class }, webEnvironment = WebEnvironment.RANDOM_PORT)
public class ChangePasswordIntegrationTest {
    ...
    @Before
    public void init() {
        Usuario user = userRepository.findByEmail("test@test.com");
        if (user == null) {
            user = new Usuario();
            user.setNombre_usuario("Test");
            user.setApellido_usuario("Test");
            user.setPassword_usuario(passwordEncoder.encode("test"));
    ...
    public void resetUserPassword() {
        final Usuario user = userRepository.findByEmail("test@test.com");
        user.setPassword_usuario(passwordEncoder.encode("test"));
        userRepository.save(user);
    }
	...
	/*
	 * @Test public void
	 * givenNotAuthenticatedUser_whenBadPasswordLoggingIn_thenCorrect() { final
	 * RequestSpecification request =
	 * RestAssured.given().auth().form("XXXXXXXX@XXXXXXXXX.com", "XXXXXXXX",
	 * formConfig).redirects().follow(false);
	 * 
	 * request.when().get("/console").then().statusCode(IsNot.not(200)).body(is(
	 * emptyOrNullString()) ); }
	 */
